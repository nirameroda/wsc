...
class LabelPropagator:
    ...
    """
    def __init__(self, graph,x,z,args):
        """
        ...
        self.lshell={node: node for node in self.nodes}
    ...
    def do_a_propagation(self):
        ...
        for node in tqdm(self.nodes):
            ...
            self.lshell[node]=self.s.shell[node-1]
            #print(lst[node])
            i=i+1
        print(i)
        print(self.lst)
        ...
        for node in tqdm(self.nodes):
            self.ni[node]=self.lst[node]+self.lshell[node]
            i=0
           # ni[node]=self.lst[node]
            neighbors = nx.neighbors(self.graph, node)
            for neighbor in tqdm(neighbors): 
                self.ni[node]=self.ni[node]+((alpha*self.lst[neighbor])/self.degree[neighbor])+(((1-alpha)*self.lshell[neighbor])/self.degree[neighbor])
